...
const Dashboard = () => {
  ...
  function LoadDisposeAssetValue() {
    ...
    try {
        ...
        .then((response) => {
          ...
          if (dataResponse == "Record Found") {
            ...
            WriteLog(
              "Error",
              "DashBoard",
              "LoadDisposeAssetValue /dispose/getAssetDisposeAmount",
      ...
      WriteLog(
        "Error",
        "DashBoard",
        "LoadDisposeAssetValue /supplier/getAssetDisposeAmount",
        "Error in try/catch \n" + err.message,
        userID
      );
  ...
  function LoadDeptCountAsset() {
    ...
    try {
      const url =
        "http://localhost:3001/department/getCountAsset_by_Department";
      axios
        .post(url)
        .then((response) => {
          const dataResponse = response.data.message;
          if (dataResponse == "Record Found") {
            ...
            WriteLog(
              "Error",
              "DashBoard",
              "LoadDeptCountAsset /department/getCountAsset_by_Department",
              response.data.message2,
              userID
            );
        ...
        .then((response) => {
          ...
          if (dataResponse == "Record Found") {
            ...
            WriteLog(
              "Error",
              "DashBoard",
              "LoadAssetDispose /dispose/getAssetDisposeAmount",
              response.data.message2,
              userID
            );
        ...
        .then((response) => {
          ...
          if (dataResponse == "Record Found") {
            ...
            WriteLog(
              "Error",
              "DashBoard",
              "LoadAssetDispose /CheckInPullOut/Count_CheckInPullOut",
              response.data.message2,
              userID
            );
                ...
                <CCol xs={6}>
                  <CCard className="mb-4">
                    ...
                    <CCardBody>
                      <CChartBar
                        data={{
                          labels: supplierasset?.map(
                            (supplier) => supplier.name
                          ),
                          datasets: [
                            {
                              label: "Asset Value",
                              backgroundColor: supplierasset?.map((supplier) =>
                                randomColor()
                              ),
                              //'#f87979',
                              data: supplierasset?.map(
                                (supplier) => supplier.assetvalue
                              ),
                            },
                          ],
                        }}
                      ...
                      <CChartBar
                        data={{
                          ...
                          datasets: [
                            {
                              label: "Asset Value",
                              backgroundColor: deptasset?.map((dept) =>
                                randomColor()
                              ),
                              //'#f87979',
                              data: deptasset?.map(
                                (dept) => dept.assetcount_department
                              ),
                            },
                          ],
                        }}
                ...
                <CAccordion flush={false}>
                  <CAccordionItem itemKey={1}>
                    <CAccordionHeader>
                      ...
                      -
                      <span className="message" style={{ color: "#ed8b13" }}>
                        {" "}
                        <> Checkout </>{" "}
                      </span>
                      -
                    ...
                    <CAccordionBody>
                      ...
                      <CCol>
                        <CChartLine
                          ...
                          data={{
                            labels: checkinpullout?.map(
                              (checkinpullout) => checkinpullout.MonthName
                            ),
                            datasets: [
                              {
                                ...
                                pointHoverBackgroundColor:
                                  getStyle("--cui-success"),
                                borderWidth: 2,
                                data: checkinpullout?.map(
                                  (checkinpullout) =>
                                    checkinpullout.CheckInCount
                                ),
                                fill: true,
                              },
                              {
                              ...
                              },
                              {
                                ...
                                borderWidth: 2,
                                data: checkinpullout?.map(
                                  (checkinpullout) => checkinpullout.Dispose
                                ),
                              },
                            ],
                          }}
                        ...
                        <CCol xs={3}>
                          <CCard className="mb-4">
                            ...
                            <CCardBody>
                              <CChartPie
                                data={{
                                  labels: assetdispose?.map(
                                    (assdispose) => assdispose.ADName
                                  ),
                                  datasets: [
                                    {
                                      data: assetdispose?.map(
                                        (assdispose) => assdispose.totamount
                                      ),
                                      backgroundColor: assetdispose?.map(
                                        (assdispose) => randomColor()
                                      ),
